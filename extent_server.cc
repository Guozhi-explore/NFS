...
int extent_server::put(extent_protocol::extentid_t id, std::string buf,std::string url, int &)
{
  ...
  if(url!="")
  {
    ...
    {
      int r;
      handle(cache_list[id].content_cached_url).safebind()->call(extent_protocol::revoke,id,
      extent_protocol::DELETE,r);
    }
...
int extent_server::getattr(extent_protocol::extentid_t id,std::string url, extent_protocol::attr &a)
{
  ...
  {
      int r;
      handle(cache_list[id].attr_cached_url).safebind()->call(extent_protocol::revoke,id,
      extent_protocol::WRITEBACK,r);
